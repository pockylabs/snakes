def is_alphabet(char):
    ...
    return char in alphabets

def count_vowels_consonants(baris):
    vowels = "aeiouAEIOU"
    count_vowels = sum(1 for char in baris if is_alphabet(char) and char.lower() in vowels)
    count_consonants = sum(1 for char in baris if is_alphabet(char) and char.lower() not in vowels)
