...
# get the vector y
def entropy(y):
    hist = np.bincount(y) # calculate the number of occurences of all class labels
    ps = hist / len(y)
...
class DecisionTree:
    ...
    def fit(self, X, y):
        # grow tree
        self.n_feats = X.shape[1] if not self.n_feats else min(self.n_feats, X.shape[1]) # to make sure it never be greater than the actual number of features
    ...
    def _split(self, X_column, split_threh):
        left_idxs = np.argwhere(X_column <= split_threh).flatten() # 1 D Vector
